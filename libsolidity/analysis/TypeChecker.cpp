...
bool TypeChecker::visit(VariableDeclaration const& _variable)
{
	...
	{
		if (
			_variable.referenceLocation() ==
			VariableDeclaration::Location::CallData ||
...
void TypeChecker::endVisit(TryStatement const& _tryStatement)
{
	...
	for (auto const& clause: _tryStatement.clauses() | ranges::views::drop_exactly(1) | views::dereferenceChecked)
	{
		if (clause.errorName() == "")
		{
			...
			{
				if (
					clause.parameters()->parameters().size() !=
					1 ||
...
{
	if (
		*_emit.eventCall().annotation().kind !=
		FunctionCallKind::FunctionCall ||
		type(_emit.eventCall().expression())->category() !=
...
void TypeChecker::endVisit(RevertStatement const& _revert)
{
	...
	if (
		*errorCall.annotation().kind !=
		FunctionCallKind::FunctionCall ||
		type(errorCall.expression())->category() !=
		Type::Category::Function ||
...
void TypeChecker::typeCheckFallbackFunction(FunctionDefinition const& _function)
{
	...
	{
		if (
			_function.returnParameters().size() !=
			1 ||
			*type(*_function.returnParameters().front()) !=
...
void TypeChecker::typeCheckABIEncodeCallFunction(FunctionCall const& _functionCall)
{
	...
	)
	{
		...
		if (externalFunctionType->hasDeclaration())
		{
			...
			if (
				externalFunctionType->declaration().visibility() ==
				Visibility::Public &&
...
void TypeChecker::typeCheckFunctionGeneralChecks(FunctionCall const& _functionCall, FunctionTypePointer _functionType)
{
	...
	if (arguments.size() < parameterTypes.size() || (!isVariadic && arguments.size() > parameterTypes.size()))
	{
		...
		auto [errorId, description] = [&]() -> std::tuple<ErrorId, std::string>
		{
			...
			)
			{
				...
				if (arguments.empty())
					return {6138_error,
							msg + " This function requires a single bytes argument."
	...
	bool callRequiresABIEncoding =
		...
		_functionType->kind() ==
		FunctionType::Kind::DelegateCall ||
		_functionType->kind() ==
		FunctionType::Kind::External ||
		_functionType->kind() ==
		FunctionType::Kind::Creation ||
		_functionType->kind() ==
		FunctionType::Kind::Event ||
...
bool TypeChecker::visit(MemberAccess const& _memberAccess)
{
	...
	if (
		_memberAccess.expression().annotation().type->category() ==
		Type::Category::Address &&
		memberName ==
		"codehash" &&
...
)
{
	...
	for (Declaration const* declaration: _candidates)
	{
		...
		if (
			uniqueDeclarations.end() == find_if(
											...
											[&](Declaration const* d)
											{
												...
												return newFunctionType &&
													   functionType->hasEqualParameterTypes(*newFunctionType);
											}
										)
		)
